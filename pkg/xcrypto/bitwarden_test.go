...
func TestCrypto(t *testing.T) {
	...
	// MasterPasswordHash
	hash := MakePassworkHash(password, dk)
	...
	mk, err := NewCryptoKey(mkb, AesCbc256HmacSha256B64)
	if err != nil {
		t.Error(err)
	}

	if mk.EncryptionType != AesCbc256HmacSha256B64 {
	...
	if string(d) != encdataDecrypted {
		t.Errorf("Expected %v got %v", string(d), encdataDecrypted)
	}
}
